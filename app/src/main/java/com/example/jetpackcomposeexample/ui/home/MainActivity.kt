...
import com.example.jetpackcomposeexample.ui.statewidget.Fetching
import com.example.jetpackcomposeexample.ui.statewidget.FetchingError
import com.example.jetpackcomposeexample.ui.statewidget.LoadMore
        ...
        Surface(
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(weight = 1f)) {
                    Text(
                        text = "Id: ${itemUserModel.id} Name: ${itemUserModel.userName}",
                        color = MaterialTheme.colors.primary,
                        style = TextStyle(fontWeight = FontWeight.Bold)
                    )
                    Text(
                        text = itemUserModel.desc,
                        color = MaterialTheme.colors.primaryVariant,
                        maxLines = if (itemUserModel.isExpanded) Int.MAX_VALUE else 1
                    )
                }
                IconButton(
...
fun ListOfUser(viewModel: MainActivityViewModel = androidx.lifecycle.viewmodel.compose.viewModel()){
    when(val placeholderState = viewModel.placeholderState.value){
        is PlaceholderState.Completed -> {
            LazyColumn(){
                itemsIndexed(viewModel.listUserModel){ index,userProfile ->
                    Greeting(itemUserModel = userProfile,viewModel = viewModel)
                    if(placeholderState.isHaveLoadMore){
                        if(viewModel.listUserModel.size - 1 == index){
                            LoadMore()
                            viewModel.loadMoreListUserModel()
                        }
                    }
                }
            }
